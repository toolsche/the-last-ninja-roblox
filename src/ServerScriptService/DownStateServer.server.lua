...
-- Verantwortlich für: Health, Timer, ProximityPrompt, Events
-- Bewegungsblockierung liegt im DownStateClient (LocalScript)
...
local function setupCharacter(player, character)
	...
	humanoid.HealthChanged:Connect(function(hp)
		if hp <= 0 and not isDown.Value then
			...
			-- Heartbeat neu starten für diesen Down-Zyklus
			local hbConn
			...
			downTimers[player.UserId] = task.delay(DOWN_DURATION, function()
				if isDown.Value then
					isDown.Value = false
					removePrompt(character)
					-- Tod durch LoadCharacter – sauberer als Health=0
					player:LoadCharacter()
					print(("[Down] %s nicht gerettet – respawn"):format(player.Name))
				end
			end)
		end
	end)
